...
# import auto_scraper  # DISABLED: Scraper stopped
# from userbot import start_userbot_worker  # DISABLED: Old leecher stopped
# from t2t_userbot import start_t2t_worker  # DISABLED: We will run it in a separate terminal
...
JSON:"""
        ...
        for key in gemini_keys:
            try:
                ...
                response = await run_async(client.models.generate_content, model='gemini-2.0-flash', contents=contents)
...
def setup_database():
    ...
    try:
        ...
        # 👇👇👇 NAYA TABLE: Torrent Queue (pending_leech) 👇👇👇
        cur.execute("""
            CREATE TABLE IF NOT EXISTS pending_leech (
                id SERIAL PRIMARY KEY,
...
def fix_movies_title_constraint():
    ...
    try:
        cur = conn.cursor()
        cur.execute("SELECT 1 FROM pg_constraint WHERE conname = 'movies_title_unique';")
        if not cur.fetchone():
...
async def keep_alive_worker():
    """Background task to keep the Render server awake by pinging its own URL."""
    logger.info("🚀 Keep-Alive Worker Initialized.")
    
    # Get the URL to ping. Prefer PUBLIC_URL if defined, otherwise fallback to local Flask
    port = os.environ.get('PORT', 8080)
    public_url = os.environ.get('PUBLIC_URL')
    
    # Fallback to local ping if PUBLIC_URL is not set
    ping_url = public_url if public_url else f"http://127.0.0.1:{port}/"
    ...
    while True:
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(ping_url, timeout=15) as response:
                    if response.status == 200:
...
# ==================== AUTO-SCRAPER WORKER (REMOVED) ====================
# Old auto_scraper worker has been fully replaced by T2T Worker.
# T2T Worker now runs with its own STRICT Clock Sync scheduling.
...
def run_telegram_bot(token, i):
    """Runs a single Telegram Bot instance safely in a thread."""
    ...
    loop.create_task(auto_delete_worker(app))
    ...
    if i == 0:
        ...
        # loop.create_task(start_t2t_worker())  # DISABLED: Run separately
...
def main():
    """Main function to start Database, Bots in Threads, and Flask Server"""
    ...
    # Khali tokens filter karo aur deduplicate karo
    ...
    # =================================================================
    # 3. Initialize & Start All Bots in Separate Threads
    # =================================================================
    logger.info(f"🤖 Found {len(tokens)} tokens. Starting threads...")
    threads = []
    
    for i, token in enumerate(tokens):
        t = threading.Thread(target=run_telegram_bot, args=(token, i))
        t.daemon = True
        t.start()
        threads.append(t)
    ...
    import subprocess
    import sys
    logger.info("🤖 Starting T2T Userbot as a background process...")
    try:
